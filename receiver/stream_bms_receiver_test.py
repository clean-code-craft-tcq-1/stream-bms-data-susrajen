...
class test_bms_stream_receiver(unittest.TestCase):
    ...
    def test_to_check_false_cal_values(self):
        ...
        self.assertFalse(cal_para_values.cal_values([0,1,2,3]) == { 'min' :0, 'max': 2})
        self.assertFalse(cal_para_values.cal_values([0,1,2,3]) == { 'min' :1, 'max': 2})
        self.assertFalse(cal_para_values.cal_values([0,0.01,0.1232,0.2583,3]) == { 'min' :1, 'max': 2, 'simple moving average':0.656124} )
        self.assertFalse(cal_para_values.cal_values([0,0.01,0.1232,0.2583,3]) == { 'min' :0, 'max': 3, 'simple moving average':0.656124} )
    ...
    def test_to_check_min_max_smavg_output(self):
        ...
        self.assertTrue(cal_para_values.min_max_smavg([0,1,2,3],[0,2,4,4],[0,1,0.1,0.3]) == { 'temperature' :{ 'max': 3, 'min' :0 },'chargeRate':{ 'min' :0, 'max': 1} ,'soc':{ 'min' :0, 'max': 4.0}})
